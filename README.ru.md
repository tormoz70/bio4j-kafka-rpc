...
[English README](README.md) · [Пример](https://github.com/tormoz70/bio4j-kafka-rpc-example) · [Релизы](https://github.com/tormoz70/bio4j-kafka-rpc/releases)
...
Координаты: `ru.sbrf.uamc:<module>:1.7`
...
Как в отдельном проекте [bio4j-kafka-rpc-example](https://github.com/tormoz70/bio4j-kafka-rpc-example): подключаете `kafka-rpc-spring-boot-starter` (или runtime + свои конфиги), настраиваете protobuf-плагин один раз. Из одних и тех же `.proto` получаете и stub'ы для клиента, и `ServiceBase` для сервера; в приложении используете и те, и другие (конфигурация через `kafka-rpc.clients.*` и `kafka-rpc.service.*` в `application.yml`).
...
Reply topic всегда берётся из заголовка клиентского запроса; серверу он в конфиге не нужен.
Имя сервиса по умолчанию — имя proto-сервиса в нижнем регистре (например `greeter`); топик запросов резолвится из `kafka-rpc.service.<name>.request-topic` автоконфигурацией.
...
Запуск сервера и клиента ([bio4j-kafka-rpc-example](https://github.com/tormoz70/bio4j-kafka-rpc-example)):
