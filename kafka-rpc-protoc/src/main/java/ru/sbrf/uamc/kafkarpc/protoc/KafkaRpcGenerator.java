...
public class KafkaRpcGenerator {
    ...
    private static final ClassName ABSTRACT_KAFKA_RPC_CHANNEL = ClassName.get("ru.sbrf.uamc.kafkarpc", "AbstractKafkaRpcChannel");
    ...
    private static final ClassName KAFKA_RPC_PROPERTIES = ClassName.get("ru.sbrf.uamc.kafkarpc.spring", "KafkaRpcProperties");
    private static final ClassName KAFKA_RPC_PROPERTIES_SERVICE = ClassName.get("ru.sbrf.uamc.kafkarpc.spring", "KafkaRpcProperties", "Service");
        ...
        for (String name : request.getFileToGenerateList()) {
            ...
            for (DescriptorProtos.ServiceDescriptorProto service : file.getServiceList()) {
                ...
                builder.addFile(PluginProtos.CodeGeneratorResponse.File.newBuilder()
                        ...
                        .build());
                String channelClassName = service.getName() + "RpcChannel";
                JavaFile channelFile = generateRpcChannel(service, outPkg);
                builder.addFile(PluginProtos.CodeGeneratorResponse.File.newBuilder()
                        .setName(outPkg.replace('.', '/') + "/" + channelClassName + ".java")
                        .setContent(channelFile.toString())
                        .build());
    ...
    private static JavaFile generateRpcChannel(DescriptorProtos.ServiceDescriptorProto service, String outPkg) {
        String channelClassName = service.getName() + "RpcChannel";
        String clientName = lowerFirst(service.getName());
        MethodSpec ctor = MethodSpec.constructorBuilder()
                .addModifiers(Modifier.PUBLIC)
                .addParameter(KAFKA_RPC_PROPERTIES, "properties")
                .addStatement("super(properties.getProducerPropertiesForClient($S), properties.getConsumerPropertiesForClient($S), properties.getRequestTopicForClient($S), properties.getReplyTopicForClient($S), properties.getTimeoutMsForClient($S))",
        ...
        TypeSpec channelType = TypeSpec.classBuilder(channelClassName)
                ...
                .addJavadoc("Channel for $L service. Config from application.yml (kafka-rpc.clients.$L).\n@deprecated Use {@link $T} via pool instead.", service.getName(), clientName, ClassName.get("ru.sbrf.uamc.kafkarpc", "PooledKafkaRpcChannel"))
                .addMethod(ctor)
                .build();
    ...
    private static JavaFile generateStubProvider(DescriptorProtos.ServiceDescriptorProto service, String outPkg) {
        ...
        MethodSpec getStub = MethodSpec.methodBuilder("getStub")
                .addModifiers(Modifier.PUBLIC)
                .returns(stubType)
                .addStatement("return new $T(pool.getOrCreate($S))", stubType, clientName)
                .build();
        MethodSpec getAsyncStub = MethodSpec.methodBuilder("getAsyncStub")
                .addModifiers(Modifier.PUBLIC)
                .returns(asyncStubType)
                .addStatement("return new $T(pool.getOrCreate($S))", asyncStubType, clientName)
                .build();
        MethodSpec ctor = MethodSpec.constructorBuilder()
                .addModifiers(Modifier.PUBLIC)
        ...
        TypeSpec providerType = TypeSpec.classBuilder(providerClassName)
                ...
                .addJavadoc("Provides $L RPC stub (channel and pool hidden). Inject and call getStub().method(...) or getAsyncStub().methodAsync(...).", service.getName())
                .addMethod(ctor)
                .addMethod(getStub)
